  ...
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .gold-btn {
      ...
      border: none; border-radius: 8px; padding: 9px 18px; font-family: 'DM Sans', sans-serif;
      font-weight: 600; font-size: 13px; cursor: pointer; transition: opacity .2s, transform .1s;
    }
...
const LoadingScreen = () => (
  <div style={{ height: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16 }}>
    ...
    <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 24, color: "var(--cream)" }}>SalonPro</div>
    <div style={{ fontSize: 13, color: "var(--muted)" }}>Connecting to database<span className="spin" style={{ marginLeft: 6 }}>◌</span></div>
  </div>
);
...
const Dashboard = ({ bookings, clients, services, salonName }) => {
  ...
  const svcPopularity = services.map(s => ({
    ...s, count: bookings.filter(b => b.service_id === s.id).length
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        <div className="card fade-up">
          ...
          {todayBookings.length === 0 && <p style={{ color: "var(--muted)", fontSize: 14 }}>No bookings today. Add one from the Calendar!</p>}
          ...
          {timeSlots.map(slot => (
            <div key={slot} style={{ display:"grid", gridTemplateColumns:"64px repeat(7,1fr)", borderBottom:"1px solid var(--border)", minHeight:52 }}>
              ...
              {days.map((d, di) => {
                ...
                const isToday = fmt(d) === todayStr;
                return (
                  <div key={di} onClick={() => openModal(d, slot)}
                    ...
                    {dayBks.map(b => {
                      ...
                      return (
                        <div key={b.id} onClick={e => e.stopPropagation()}
                          style={{ background:svc?.color?`${svc.color}33`:"rgba(201,168,76,.2)", border:`1px solid ${svc?.color||"var(--gold)"}66`, borderRadius:6, padding:"3px 6px", marginBottom:2, fontSize:11 }}>
          ...
          <div className="card fade-up" style={{ width:420, padding:28 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
              ...
              <div>
                ...
                <textarea style={{ width:"100%", minHeight:70 }} value={form.notes} onChange={e => setForm(p=>({...p,notes:e.target.value}))} placeholder="Special requests…" />
              </div>
              <button className="gold-btn" onClick={saveBooking} disabled={saving}>
...
const Clients = ({ clients, setClients, bookings, services }) => {
  ...
  const clientBookings = (cid) => bookings.filter(b => b.client_id === cid).map(b => ({
    ...b, svc: services.find(s => s.id === b.service_id)
  ...
  const addClient = async () => {
    ...
    const { data, error } = await supabase.from("clients").insert([{
      ...
      email: newClient.email, notes: newClient.notes,
      tags: [], total_spent: 0, visits: 0, last_visit: "—"
    }]).select();
    ...
    <div style={{ display:"flex", gap:20, height:"calc(100vh - 120px)" }}>
      <div style={{ width:320, display:"flex", flexDirection:"column", gap:12 }}>
        ...
        <div style={{ flex:1, overflowY:"auto", display:"flex", flexDirection:"column", gap:8 }}>
          {filtered.length === 0 && <p style={{ color:"var(--muted)", fontSize:14, padding:8 }}>No clients yet. Add your first one!</p>}
          {filtered.map(c => (
            <div key={c.id} className="card" onClick={() => setSelected(c)}
              style={{ cursor:"pointer", padding:"14px 16px", border:selected?.id===c.id?"1px solid var(--gold)":"1px solid var(--border)", transition:"border .2s" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                <div>
...
const ServicesView = ({ services, setServices }) => {
  ...
  const addService = async () => {
    ...
    const { data, error } = await supabase.from("services").insert([{
      ...
      color: CAT_COLORS[form.category]||"#c9a84c"
    }]).select();
...
const AIHub = ({ clients, bookings, services, salonName }) => {
  ...
  const [chatHistory, setChatHistory] = useState([
    { role:"assistant", content:`Hi! I'm your SalonPro AI for **${salonName}**. I can write follow-up messages, upsell campaigns, and more. What would you like to do?` }
  ]);
  ...
  const generateMessage = async () => {
    ...
    const prompt = `You are a professional salon assistant for "${salonName}". Write a ${msgType==="email"?"personalized email":msgType==="sms"?"short SMS (max 160 chars)":"WhatsApp message"} for: "${aiGoal}".
Client: ${client.name}, last visit: ${client.last_visit} (${lastSvc?.name||"unknown"}), ${client.visits||0} visits, notes: ${client.notes||"none"}.
Available services: ${allSvcs}.
${msgType==="email"?"Include subject line as first line starting with 'Subject:'.":""}
Write only the message, be warm and personal.`;
    ...
    try {
      ...
      setResult(data.content?.[0]?.text || "Error generating.");
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          system:`You are SalonPro AI for "${salonName}". Clients: ${clients.map(c=>c.name).join(", ")}. Services: ${services.map(s=>s.name).join(", ")}.`,
          messages: newHistory.map(m=>({role:m.role,content:m.content}))
        })
      });
      const data = await res.json();
      setChatHistory(p => [...p, {role:"assistant",content:data.content?.[0]?.text||"Sorry, error."}]);
  ...
  const GOALS = [
    ...
    {value:"win-back",label:"Win-back (inactive)"},
    {value:"birthday offer",label:"Birthday special offer"},
    {value:"new service announcement",label:"New service announcement"},
  ];

  const tabs = [{id:"followup",label:"Message Generator",icon:"mail"},{id:"chat",label:"AI Assistant",icon:"bot"}];
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        ...
        <div style={{ display:"flex", padding:4, background:"var(--surface2)", borderRadius:10, gap:2 }}>
          {tabs.map(t => (
            <button key={t.id} onClick={() => setActiveTab(t.id)} style={{ display:"flex", alignItems:"center", gap:6, padding:"7px 14px", borderRadius:8, border:"none", background:activeTab===t.id?"var(--surface)":"transparent", color:activeTab===t.id?"var(--gold)":"var(--muted)", cursor:"pointer", fontSize:13, fontFamily:"'DM Sans',sans-serif", fontWeight:500 }}>
      ...
      {activeTab==="followup" && (
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20 }}>
          <div className="card" style={{ display:"flex", flexDirection:"column", gap:16 }}>
            ...
            <button className="gold-btn" onClick={generateMessage} disabled={!selectedClient||loading}>
      ...
      {activeTab==="chat" && (
        <div className="card" style={{ display:"flex", flexDirection:"column", height:"calc(100vh - 220px)" }}>
          <div style={{ flex:1, overflowY:"auto", display:"flex", flexDirection:"column", gap:16, padding:"4px 0", marginBottom:12 }}>
            {chatHistory.map((msg, i) => (
              <div key={i} style={{ display:"flex", justifyContent:msg.role==="user"?"flex-end":"flex-start" }}>
                ...
                <div style={{ maxWidth:"72%", padding:"12px 16px", borderRadius:msg.role==="user"?"16px 16px 4px 16px":"16px 16px 16px 4px", background:msg.role==="user"?"linear-gradient(135deg,var(--gold),var(--gold-lt))":"var(--surface2)", color:msg.role==="user"?"#1a1410":"var(--text)", fontSize:14, lineHeight:1.6, whiteSpace:"pre-wrap" }}>
                  {msg.content}
                </div>
              </div>
            ))}
          ...
          <div style={{ display:"flex", gap:10 }}>
            <input style={{ flex:1 }} placeholder="Ask anything… e.g. 'Write a win-back campaign for clients not seen in 60 days'" value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => e.key==="Enter"&&!e.shiftKey&&sendChat()} />
...
const Settings = ({ salonName, setSalonName }) => {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:20, maxWidth:680 }}>
      ...
      <div className="card fade-up">
        <h3 style={{ fontSize:17, color:"var(--cream)", marginBottom:16 }}>Salon Information</h3>
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          <div>
            <label style={{ fontSize:12, color:"var(--muted)", marginBottom:4, display:"block" }}>Salon Name</label>
      ...
      <div className="card fade-up">
        <h3 style={{ fontSize:17, color:"var(--cream)", marginBottom:8 }}>Database</h3>
        <div style={{ display:"flex", alignItems:"center", gap:10, padding:"12px 14px", background:"var(--surface2)", borderRadius:10 }}>
          <div style={{ width:8, height:8, borderRadius:"50%", background:"var(--green)" }} />
          <span style={{ fontSize:14, color:"var(--cream)" }}>Connected to Supabase</span>
